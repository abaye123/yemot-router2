...
function validateCharsForTTS (text) {
    const invalidCharsRgx = /[.\-"'&|]/g;
    const invalidCharsMatched = text.match(invalidCharsRgx);
    if (invalidCharsMatched) {
        throw new Error(`message '${text}' has invalid characters for yemot: ${colors.red(invalidCharsMatched.join(', '))}`);
    }
}
...
function makeMessagesData (messages, options) {
