...
function makeTapModeRead (messagesCombined, options) {
    ...
    if (options.digits_allowed && !Array.isArray(options.digits_allowed)) {
        throw new InputValidationError(`digits_allowed must be array, got ${typeof options.digits_allowed} (${options.digits_allowed})`);
    }
    ...
    for (const value of messages) {
        ...
        case 'system_message': {
            ...
            if (sysMsgId.length !== 4) {
                throw new InputValidationError(`'${value.data}' is not a valid system message id - it should be 4 digits, got ${sysMsgId.length}!`);
            }
            res += sysMsgId;
            i++;
            break;
