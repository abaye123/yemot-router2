...
function makeReadData (messages, silentRemoveInvalidChars = false) {
    for (const msg of messages) {
        if (typeof msg.data !== 'string') continue;
        if (msg.type === 'text' && (msg.silentRemoveInvalidChars || silentRemoveInvalidChars)) {
...
function makeReadResponse (messages, mode, options, valIndex = 1) {
    ...
    let responseText;
    const messagesCombined = makeReadData(messages, options.silentRemoveInvalidChars);
