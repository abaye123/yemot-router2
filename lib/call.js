...
class Call {
    ...
    async read (messages, mode = 'tap', options = {}) {
        ...
        const sendResp = async () => {
            ...
            const messagesCombined = makeMessagesData(messages, {
                removeInvalidChars: options.removeInvalidChars ?? this.#defaults.read.removeInvalidChars
            });
            ...
            for (const [oldName, newName] of readOptionsDeprecats) {
                if (typeof readOptions[oldName] !== 'undefined') {
                    throw new Error(`read option '${oldName}' is deprecated, use '${newName}' instead`);
                }
            }
    ...
    */
    ...
    get query () {
        throw new Error('call.query is deprecated, use call.values instead');
    }

    get body () {
        throw new Error('call.body is deprecated, use call.values instead');
    }

    get params () {
        throw new Error('call.params is deprecated, use call.req.params instead');
    }
}
